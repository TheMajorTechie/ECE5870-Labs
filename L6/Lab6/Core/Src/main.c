...
void transmit_int(int intToSend) {
	//check and wait on USART status flag indicating transmit register empty
...
//configure GPIOA's mode to analog function
	...
	ADC1->CFGR1 |= (2 << 3);	//set resolution to 8 bits
	ADC1->CFGR1 |= ADC_CFGR1_CONT;
	ADC1->CFGR1 &= ~(3 << 10);	//set to software triggering
