...
//	uint32_t debouncer = 0;
	...
	EXTI->IMR |= (1);
	//now set it to have a rising edge trigger
	EXTI->RTSR |= (1);
	EXTI->FTSR &= ~(1);
	
	//set PA0 to be routed to the EXTI0 input line 0
	SYSCFG->EXTICR[0] &= ~(1);
  ...
  {
		HAL_Delay(500);
		GPIOC->ODR ^= (1 << 6);			//invert the red LED every 500ms
