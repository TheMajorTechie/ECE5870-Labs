...
void encoder_init(void) {
    ...
    GPIOB->AFR[0] |= ( (1 << 16) | (1 << 20) );											//PB4 and PB5
...
void PI_update(void) {
     ...
     */
		...
		//convert motor speed from RPM to encoder counts
		//using a 5:1 ratio between encoder counts and revolutions
		
		int16_t target_encnt = 2*target_rpm;
		
		error = target_encnt - motor_speed;
    ...
    int16_t output = Kp * error + error_integral; // raw output
